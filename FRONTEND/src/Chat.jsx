...
//                     const response = await fetch(`${API_URL}/thread/${latestThread.threadId}`, {
// =======
...
function Chat() {
    ...
    // Load latest thread after login
    useEffect(() => {
        if (!currThreadId && allThreads?.length) {
            const latestThread = allThreads[0]; // most recent thread
            setCurrThreadId(latestThread.threadId);
    ...
    return (
        <>
            ...
            <div className="chats">
                {/* Render all except last (assistant typing effect will handle last) */}
                {prevChats?.slice(0, -1).map((chat, idx) => (
                    <div
                ...
                {/* Last assistant message with typing animation */}
                {prevChats.length > 0 && (
                    <>
                        {latestReply === null ? (
                            <div className="gptDiv" key="static">
                                <ReactMarkdown rehypePlugins={[rehypeHighlight]}>
                                    {prevChats[prevChats.length - 1].content}
                                </ReactMarkdown>
                            </div>
